...
let[@alloc a] basename_at string ~non_negative_final_slash_index:index =
  (String.drop_prefix [@alloc a]) string (index + 1) [@exclave_if_stack a]
;;

let[@alloc a] dirname_at string ~non_negative_final_slash_index:index =
  if index = 0
  then root
...
let[@alloc a] dirname string ~(if_some : _ -> _) ~(if_none : _ -> _) =
...
let[@mode l] foldr =
  ...
  and foldr_up_to_part_end
    ...
    ~state
    ~acc
    ~(f : _ -> pos:_ -> len:_ -> state:_ -> acc:_ -> _)
    =
    let pos = start + 1 in
...
let[@alloc a] to_parts =
  let add_part string ~pos ~len ~state:(part_of_string : _ -> _) ~acc =
    (let part_string = (String.sub [@alloc a]) string ~pos ~len in
     part_of_string part_string :: acc)
    [@exclave_if_stack a]
  in
  let to_parts string ~(part_of_string : _ -> _) =
    (foldr [@mode l])
      string
